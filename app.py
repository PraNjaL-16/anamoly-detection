...
def main():
    ...
    activities = ["Home", "Text Polarity","Twitter sentiment analysis tool", "Hate speech detection", "Emotion recogination", "Youtube scraping", "Whatsapp"]
    
    choice = st.sidebar.selectbox("Select Your Activity",activities)
    
    if choice == "Home":
        st.title("🔥 TEXT Sentiment analysis tool 🔥")
        st.subheader("Our project contains SIX functionalities created with the help of \n1. basic MACHINE LEARNING alogrithums, techniques & trained models \n2. python libraries \n3. third party APIs \n4. python codes \n5. _streamlit_ for web development")
        
    elif choice == "Text Polarity":
        
        st.title("🔥 Text Sentiment Analysis 🔥")
        
        st.subheader("model will return the polarity of texual data")
        
        raw_text = st.text_area("Please, enter the text", height=100)
        
        analyze_0 = st.button("Get Polarity", key='analyze_0')
        
        if analyze_0:
            
            st.success("Analyzing the text")
            ...
            polarity = getPolarity(processed_text)
            emotion = getAnalysis(polarity)
            
            
            if emotion == 'Positive':
                    st.write("Text contains **POSITIVE** emotion")
            elif emotion == 'Negative':
                st.write("Text contains contains **NEGATIVE** emotion")
            elif emotion == 'Neutral':
                st.write("Text is **NEUTRAL** in nature")
        ...
        if analyze_9:
            ...
            st.success('**comments & their polarity**')
            ...
            for comment in df['Comment']:
                try:    
                    ...
                    polarity = getPolarity(comment)
                    emotion = getAnalysis(polarity)
                    
                    count += 1
                    st.write(comment + ' **- ' + emotion + '**')
            ...
            st.success('**' + str(count) + '** comments are analyzed')
        ...
        if analyze_10:
            ...
            st.success('**' + str(count) + '** messages are analyzed')
            ...
            neuvals = neutral
            reacts3 = ax.bar(ind+width+width, neuvals, width, color='b')
            
            ax.set_xlabel('Names')
            ax.set_ylabel('Sentiment')
            
            ax.set_xticks(ind+width)
            ax.set_yticks(np.arange(0,max_x+50,10))
            ax.set_xticklabels( names )
            ax.legend( (rects1[0], rects2[0], reacts3[0]), ('positive', 'negative', 'neutral') )
